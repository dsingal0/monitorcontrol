...
class MenuHandler: NSMenu, NSMenuDelegate {
  ...
  func addLayoutsSubmenu() {
    let layouts = LayoutManager.shared.layouts
    let activeLayout = LayoutManager.shared.activeLayout
    
    let layoutsMenuItem = NSMenuItem(title: NSLocalizedString("Layouts", comment: "Shown in menu"), action: nil, keyEquivalent: "")
    let layoutsSubmenu = NSMenu()
    
    for layout in layouts {
      let layoutItem = NSMenuItem(title: layout.name, action: #selector(self.applyLayout(_:)), keyEquivalent: "")
      layoutItem.representedObject = layout
      layoutItem.target = self
      if activeLayout?.id == layout.id {
        layoutItem.state = .on
      }
      layoutsSubmenu.addItem(layoutItem)
    }
    
    if !layouts.isEmpty {
      layoutsSubmenu.addItem(NSMenuItem.separator())
    }
    
    let saveLayoutItem = NSMenuItem(title: NSLocalizedString("Save Current Layout…", comment: "Shown in menu"), action: #selector(self.saveCurrentLayout), keyEquivalent: "")
    saveLayoutItem.target = self
    layoutsSubmenu.addItem(saveLayoutItem)
    
    let manageLayoutsItem = NSMenuItem(title: NSLocalizedString("Manage Layouts…", comment: "Shown in menu"), action: #selector(self.manageLayouts), keyEquivalent: "")
    manageLayoutsItem.target = self
    layoutsSubmenu.addItem(manageLayoutsItem)
    
    layoutsMenuItem.submenu = layoutsSubmenu
    self.insertItem(layoutsMenuItem, at: self.items.count)
  }
  
  @objc func applyLayout(_ sender: NSMenuItem) {
    guard let layout = sender.representedObject as? DisplayLayout else { return }
    os_log("Applying layout '%{public}@'", type: .info, layout.name)
    do {
      try LayoutManager.shared.applyLayout(layout)
      self.updateMenus()
    } catch {
      os_log("Failed to apply layout: %{public}@", type: .error, error.localizedDescription)
      let alert = NSAlert()
